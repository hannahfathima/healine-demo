...
const EstablishmentTypes = (props) => {
  const {
    professionsList,
  ...
  const [establishmentTypesDataGridOptions, setEstablishmentTypesDataGridOptions] = useState({
    loading: false,
    rows: [],
    totalRows: 0,
  ...
  const getEstablishmentTypes = useCallback(async () => {
    ...
    const result = await fetchList(
      ApiEndPoints.ESTABLISHMENT_TYPES +
  ...
  const editEstablishmentTypes = async (id) => {
    setId(id)
    setOpen(true)
    const name = establishmentTypesDataGridOptions.rows.find((item)=>item.id === id)
  ...
  return (
    <div className="min-width">
      ...
      >
        <Typography variant="h4">Establishment Types</Typography>
        <Button variant="outlined" onClick={() => setOpen(true)}>
          Add Establishment Types
        </Button>
      </div>
      ...
      <div style={{ height: "75vh", width: "100%" }}>
        <DataGrid
          ...
          rowsPerPageOptions={establishmentTypesDataGridOptions.rowsPerPageOptions}
      ...
      <ConfirmDialog
        ...
        message="Do you really want to delete the Establishment Type?"
      ...
      <Dialog open={open} onClose={handleClose}>
        <DialogTitle>{id ? 'Edit Establishment Types' : ' Add Establishment Types'}</DialogTitle>
        <DialogContent>
          <TextField
            autoFocus
            margin="dense"
...
export default connect(mapStateToProps, mapDispatchToProps)(EstablishmentTypes);
