...
function ManageEstablishmentHours(props) {
    ...
    // },
    {
      ...
      renderCell: (params) => (
        <>
          <CustomIconButton
            onClickAction={() => {
              navigate(`/establishments/manage-hours/edit/${params.id}`);
            }}
            arialLabel="edit"
            icon={<EditIcon />}
          />
  ...
  useEffect(() => {
    setDataGridData(estWorkingHoursList);
  }, [estWorkingHoursList]);
  ...
  const getWorkingHoursList = useCallback(async () => {
    ...
    if (result.status === 200) {
      ...
      estWorkingHoursListSuccess(result.data.rows);
    ...
    <div className="min-width">
      ...
      >
        <Button
          variant="outlined"
          onClick={() =>
            navigate(`/establishments/manage-hours/add/${establishmentId}`)
          }
        >
          Add Working Hours
        </Button>
...
const mapStateToProps = (state) => ({
  ...
  estWorkingHoursList: state.establishment.estWorkingHoursList,
});
...
const mapDispatchToProps = (dispatch, ownProps) => {
  return {
    ...
    estWorkingHoursListSuccess: (data) =>
      dispatch(estWorkingHoursListSuccess(data)),
    estWorkingHoursListFailed: () => dispatch(estWorkingHoursListFailed()),
  };
};
